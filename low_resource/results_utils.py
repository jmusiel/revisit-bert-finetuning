...
def get_test_accs(results, dataset, method, filetype, split, replicate):
    test_metrics = get_test_metrics()
    test_metric = test_metrics[dataset]
    test_metric = test_metric[0][test_metric[1]]
    test_df = results[(results["dataset"] == dataset) & (results["method"] == method) & (results["filetype"] == filetype) & (results["split"] == split) & (results["replicate"] == replicate)]
...
def get_val_accs(results, dataset, method, filetype, split, replicate):
    val_metrics = get_val_metrics()
    val_metric = val_metrics[dataset]
    val_metric = val_metric[0][val_metric[1]]
    val_df = results[(results["dataset"] == dataset) & (results["method"] == method) & (results["filetype"] == "raw_log") & (results["split"] == split) & (results["replicate"] == replicate)]
...
def debias(val_accuracies, test_accuracies, size=5, high=5):
